...
void loadGLtexture(void)
{
	...
	glGenTextures(1, &texture_checkerboard); // empty textute obj craetes

	glBindTexture(GL_TEXTURE_2D, texture_checkerboard);

	glPixelStorei(GL_UNPACK_ALIGNMENT, 1); 

	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST); // mag means jevha object motha hoil tevha quality
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST); // Lamb ahe mhnun kami quality

	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT); // mag means jevha object motha hoil tevha quality	
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT); // mag means jevha object motha hoil tevha quality
