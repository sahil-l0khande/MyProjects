projection stack ha kadhi vapraycha ?
Every game mdhe vapartat, FPP, map kay tumhi kuthy he dakhvyl like in GTA, Picture in picture mhntat

perspective vapraych game sathi ani ortho vaprych to right side vala map dakhvtana

so jumping krtatna, save krycha game mdhla mg pip mdhe jych like that

stack mdhli stack frame, 2D sathi projection stack vapru shakto, like for map showing
...
// Robotic arm related variables (matrix stack)
int shoulder = 0;
int elbow = 0;
...
void display(void)
{
	...
	// Both arm and forearms are filled polygons
	glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);

	// DO basic translation common to both arm and forearm
	glTranslatef(0.0f, 0.0f, -12.0f);	// 12 ne mage gela ani to stack mdhe save kela

	// Push this matrix into matrix stack
	glPushMatrix(); // using this

	/* CODE FOR ARM */
	glRotatef((GLfloat)shoulder, 0.0f, 0.0f, 1.0f); // z axis rotation
