...
void display(void)
{
	...
	/* Code for earth */
	// Again save this matrix to come back 
	glPushMatrix();

	// Let the earth allow to revolve around the sun
	glRotatef((GLfloat)year, 0.0f, 1.0f, 0.0f);

	// Earth is at some dostance from sun
	glTranslatef(1.5f, 0.0f, 0.0f);

	// Adjus the pole of spheres opf earth
	glRotatef(90.0f, 1.0f, 0.0f, 0.0f);

	// Let the earth spin/rotate around itself
	glRotatef((GLfloat)date, 0.0f, 0.0f, 1.0f);

	// Polygon fill
	glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);

	// Set earth color bluish 
	glColor3f(0.4f, 0.7f, 1.0f);

	// Draw earth sphere
	gluSphere(quadric, 0.2f, 20, 20);
	glPopMatrix();
	// After drawing sphere , we go back to the original to draw earth hence pop the matrix or restore the matrix
	// swap the buffers
	SwapBuffers(ghdc);
}
