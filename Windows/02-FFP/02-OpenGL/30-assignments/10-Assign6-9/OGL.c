...
void display(void)
{
	...
	if (gbGraph == TRUE) {
		...
		for (float i = -2.2f; i < 2.2f; i += 0.1f) {  // Slightly increase range
			if (count % 5 == 0) {
				glEnd();
				glLineWidth(2.0f);
				glBegin(GL_LINES);
			...
			glVertex3f(i * 1.15f, -2.5f, 0.0f); // Adjust scaling factor
			glVertex3f(i * 1.15f, 2.5f, 0.0f);
			count++;
		}
		glEnd();
		...
		glBegin(GL_LINES);
		glColor3f(0.0f, 0.0f, 1.0f);
		for (float i = -2.0f; i < 2.1f; i = i + 0.1f) {
			if (count_2 % 5 == 0) {
				glEnd();
				glLineWidth(2.0f);
				glBegin(GL_LINES);
	...
	if (gbCircle == TRUE) {
		if (gbPoints == TRUE) {
			const int sides = 200;
			const double radius = 0.70;
			glPointSize(2.5f);
			glBegin(GL_POINTS);
			glColor3f(1.0f, 1.0f, 0.0f);
			for (int i = 0; i < 360; i += 360 / sides) {
				double heading = i * M_PI / 180;
				glVertex3f(cos(heading) * radius, sin(heading) * radius, 0.0f);
			}
			glEnd();
		}
